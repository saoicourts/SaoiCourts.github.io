...
toc_progress.getElementsByTagNames=function(list,obj)
{
	...
	for (var i=0;i<tagNames.length;i++)
	{
		...
		{
			if (tags[j].getAttribute('class')) {
				if (tags[j].getAttribute('class').split(' ').some(x => x == 'part-title')){
	...
	for (var main_sections_index=0;main_sections_index<main_sections.length;main_sections_index++)
	{
		...
		var main_title_set = false;
		{
			...
			if (title_element!=null && (!title_element.hasAttribute('class') || title_element.getAttribute('class').indexOf('no-toc-progress')==-1))
			{
				...
				style_node.textContent=style_node.textContent+'.toc-progress-'+main_sections_index.toString()+' #toc-progress-'+main_sections_index.toString()+' {font-weight: bold; color:red;}\n';
